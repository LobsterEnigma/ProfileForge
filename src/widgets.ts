import type { CareState } from "./care/state.js";
...
export function renderWidget(profile: GitHubProfile, params: PetParams, care?: Care): string {
  ...
  const pet = computePetState(profile, { petName: params.petName, species: params.species });
  return renderPetCard(care ? applyCare(pet, care.state, care.now) : pet, world(params));
}
...
export function renderDemo(params: PetParams): string {
  ...
  const extras = { trick: params.trick, away: params.away, visit: params.visit, dirt: params.dirt };
